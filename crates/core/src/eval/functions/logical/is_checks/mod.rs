...
mod isemail_tests {
    ...
    #[test]
    fn no_dot_in_domain() {
        // "user@localhost" has no dot — domain must have at least one dot
        assert!(!is_valid_email("user@localhost"));
    }
}

#[cfg(test)]
mod isemail_fn_tests {
    use super::isemail_fn;
    use crate::eval::functions::EvalCtx;
    use crate::eval::{Context, Registry};
    use crate::parser::ast::{Expr, Span};
    use crate::types::{ErrorKind, Value};

    fn span() -> Span { Span::new(0, 1) }

    fn run_isemail(expr: Expr) -> Value {
        let registry = Registry::new();
        let ctx = Context::empty();
        let mut eval_ctx = EvalCtx::new(ctx, &registry);
        isemail_fn(&[expr], &mut eval_ctx)
    }

    #[test]
    fn number_arg_returns_false() {
        let result = run_isemail(Expr::Number(42.0, span()));
        assert_eq!(result, Value::Bool(false));
    }

    #[test]
    fn bool_arg_returns_false() {
        let result = run_isemail(Expr::Bool(true, span()));
        assert_eq!(result, Value::Bool(false));
    }

    #[test]
    fn zero_args_returns_error_na() {
        let registry = Registry::new();
        let ctx = Context::empty();
        let mut eval_ctx = EvalCtx::new(ctx, &registry);
        let result = isemail_fn(&[], &mut eval_ctx);
